...
class TransformerConfig(FairseqDataclass):
    ...
    ## added paramter for simul-trans
    knowledge_distillation : str = field(
        default = None,
        metadata = {
        "help": "the file path of the pretrained teacher model"
        }
    )
    pretrained_encoder_model_path: str = field(
        default = None,
        metadata={
            "help":"if we need to use a pretrained and frozen encoder model "
        }
    )
    cons_replace:bool = field(
        default=False, metadata={"help":'whether to use lexical constrained translation in dinu paper replace method'}
    )
    cons_append:bool = field(
        default=False, metadata={"help":"whether to use lexical constrained translation in dinu paper append method"}
    )
