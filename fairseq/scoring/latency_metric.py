...
def ConsecutiveWait(
    ...
    _delays = torch.cat(
        (torch.zeros([delays.size(0), 1], device=delays.device), delays[:, :-1]), dim=1
    )
    CW = delays[:, -1:] / (delays > _delays).sum(dim=-1, keepdim=True)
...
def AverageLagging(delays, src_lens, tgt_lens, ref_lens=None, target_padding_mask=None):
    ...
    # print("oracle delays:{}".format(oracle_delays))
