...
class CommonConfig(FairseqDataclass):
    ...
    has_teacher: bool = field(
        default=False, metadata={"help": "Whether has pretrained teacher model or not"}
    )
...
class GenerationConfig(FairseqDataclass):
    ...
    confidence_decoding: bool = field(
        default=False,
        metadata={"help": "test simultaneous translation in confidence-based policy decoding"},
    )
    confidence_threshold: float = field(
        default=-1.0,
        metadata={"help": "test confidence threshold in SM2 confidence-based decoding"},
    )
    decoding_path: str = field(
        default="./",
        metadata={"help": "path for saving decoding."},
    )
