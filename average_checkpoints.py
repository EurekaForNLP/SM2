...
def last_n_checkpoints(
    paths, n, update_based, best_base, upper_bound=None, last=None, interval=None
):
    assert len(paths) == 1
    path = paths[0]
    if update_based:
    ...
    if best_base:
        pt_regexp = re.compile(r"checkpoint.best_accuracy_(\d\d.\d\d).pt")
    ...
    for f in files:
        ...
        if m is not None:
            ...
            if upper_bound is None or sort_key <= upper_bound:
                ...
                if last is not None:
                    # pdb.set_trace()
                    if m.group(0).split("_")[-1] == last.split("_")[-1]:
                        last_key = sort_key
    if len(entries) < n:
        raise Exception(
    ...
    if last_key is not None:
        ...
        for entry in new_entries:
            if interval is None:
                ...
                if entry[0] <= last_key and (last_key - entry[0]) % interval == 0:
                    newnew_entries.append(entry)
        
        return [os.path.join(path, x[1]) for x in sorted(newnew_entries, reverse=True)[:n]]
...
def main():
    ...
    if args.average_interval is not None:
        interval = int(args.average_interval)
    else:
        interval = None
    # pdb.set_trace()
    is_best_based = getattr(args, "best", False)
    if num is not None:
