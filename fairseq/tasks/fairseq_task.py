...
class FairseqTask(object):
    ...
    def build_generator(
        ...
        from fairseq.confidence_sequence_generator import ConfidenceSequenceGenerator
        ...
        if seq_gen_cls is None:
            ...
            elif getattr(args,"confidence_decoding",False):
                seq_gen_cls = ConfidenceSequenceGenerator
                extra_gen_cls_kwargs["confidence_threshold"] = args.confidence_threshold
            else:
    ...
    def train_step(
        ...
        if hasattr(criterion,'set_num_updates'):
            criterion.set_num_updates(update_num)
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                if teacher_model is not None:
                    loss, sample_size, logging_output = criterion(model, sample,teacher_model)
                else:
