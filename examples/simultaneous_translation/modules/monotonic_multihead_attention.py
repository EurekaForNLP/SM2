...
class MonotonicAttention(MultiheadAttention):
    ...
    def monotonic_attention_process_infer(
        ...
        if self.soft_attention:
            ...
            beta_mask = torch.arange(src_len).to(alpha.device).expand_as(alpha).gt(monotonic_step).unsqueeze(1)
    ...
    def monotonic_attention_process_train(
        ...
        # print("p_choose:{}".format(p_choose[0,0,:]))
    ...
    def monotonic_attention_process_test(
        ...
        if self.waitk_lagging <= 0:

            # 1. compute stepwise probability
            p_choose = self.p_choose(query, key, key_padding_mask)
            # print("p choose:{}".format(p_choose.shape))

            # 2. compute expected_alignment

            alpha = expected_alignment_from_p_choose(
                torch.round(p_choose),
                key_padding_mask,
                eps=self.eps,
            )
            ...
            p_choose = (tmp == index).type_as(query).unsqueeze(0).repeat(bsz, 1, 1)
        ...
        if self.soft_attention:
            ...
            soft_energy = soft_energy.masked_fill(
                (1 - truncated_mask).bool(), -float("inf")
            )
    ...
    def forward(
        ...
        if multipath_k is not None:
            self.waitk_lagging = multipath_k
            # print("multipath_k is not None!!!")
        if waitk_test_lagging is not None:
            self.waitk_lagging = waitk_test_lagging
        # print("waitk lagging:{}".format(self.waitk_lagging))
        #获得源端padding 的mask，注意这里只支持右padding。
        if key_padding_mask is not None:
        ...
        #与正常的multihead-attention相同，进行对v的映射计算
        v = self.v_proj(value)
        length, bsz, _ = v.size()
        v = (
            v.contiguous()
        ...
        #对模型进行注意力计算，注意，beta在此处起到了和正常的attention计算中的注意力权重矩阵相同的作用
        # print("beta:{}\nv:{}".format(beta.shape,v.shape))
        # print("beta:{}\nv:{}".format(beta[0,:,:],v[0,:,:]))
        ...
        return attn, {
            ...
            "alpha": alpha,
            "beta": beta,
            "soft_energy":soft_energy,#等同于正常的cross-attn中的attn-weight
        }
...
class WaitKAttention(
    ...
    def __init__(self, args):
        ...
        assert self.waitk_lagging > 0 or args.multipath, (
